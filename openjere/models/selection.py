...
class MultiHeadSelection(ABCModel):
    ...
    def inference(self, mask, text_list, decoded_tag, selection_logits):
        selection_mask = (
            ...
            .unsqueeze(2)
            .expand(-1, -1, len(self.relation_vocab), -1)
        )  # batch x seq x rel x seq
        selection_tags = (
            torch.sigmoid(selection_logits) * selection_mask.float()
    ...
    def masked_BCEloss(self, mask, selection_logits, selection_gold):
        ...
        selection_loss = F.binary_cross_entropy_with_logits(
            selection_logits, selection_gold, reduction="none"
        )
