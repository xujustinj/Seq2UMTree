...
class RDFFileReader:
    ...
    def fix_document(self, s_tripleset_raw, template, text, tag2ent):
        ...
        uniq_tag2ent = {
            tag: ent for tag, ent in tag2ent.items() if tag in tag2uniq_tag.values()
        }
...
class WebNLGDataReader(DataReader):
    def __init__(self, set: DataSetType):
        ...
        super().__init__(
            data, misspelling=misspelling, rephrase=(rephrase, rephrase_if_must)
        )
    ...
    def save(self):
        ...
        save_f = path.join(
            path.dirname(path.realpath(__file__)), data_set_type + ".json"
        )
...
def download():
    cmd = (
        "rm -rf data/webnlg/raw 2>/dev/null \n"
...
def cp_data():
    cmd = (
        "cp data/webnlg/train.json ./train_data.json \n"
